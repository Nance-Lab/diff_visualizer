...
@given(diffusion_coefficient=st.floats(min_value=0.1, max_value=100.0, allow_infinity=False),
       temperature=temperature_strategy,
       particle_radius=particle_radius_strategy)
@settings(max_examples=10)
def test_viscosity_from_diffusion_coefficient(diffusion_coefficient, temperature, particle_radius):
    # Calculate viscosity using the function
    test_dataframe = pd.DataFrame({'Deff1': [diffusion_coefficient]})
    viscosity_series = viscosity_from_diffusion_coefficient(test_dataframe, 'Deff1', temperature, particle_radius)
    ...
    for index, row in test_dataframe.iterrows():
        ...
        expected_viscosity = (1.380649e-23 * temperature) / (6 * math.pi * diffusion_coefficient * particle_radius)
