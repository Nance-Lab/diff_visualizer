...
def test_feature_violin_plot():
    ...
    fig = feature_violin_plot(
        testing_df, feature_to_plot="Deff1", label_column=None, figsize=(2, 2)
    )
    ...
    fig = feature_violin_plot(
        testing_df, feature_to_plot="Deff1", label_column="age", figsize=(2, 2)
    )
    assert isinstance(fig, plt.Figure)  # Check if a Figure object is returned
    assert fig.axes[0].get_title() == ""
