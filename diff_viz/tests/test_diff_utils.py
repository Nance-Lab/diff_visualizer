...
def test_get_csvs(file_path=file_path):
    ...
    assert "geomean_P17_1h_OGD_1d_40nm_slice_1_cortex_vid_1.csv" in geo_list
    assert "geomean_P17_NT_1d_40nm_slice_1_cortex_vid_1.csv" in geo_list
...
def test_get_geo_df_geomean(file_path=file_path):
    # test geomean
    geo_df = get_geo_df(
        file_path, filetype="geomean", doses=["1h", "NT"], timepoints=["1d"]
    )
...
def test_get_geo_df_geoSEM(file_path=file_path):
    # test geosem
    geo_df = get_geo_df(
        file_path, filetype="geoSEM", doses=["1h", "NT"], timepoints=["1d"]
    )
    assert geo_df.shape == (650, 2)
    assert geo_df.columns[0] == "1h_1d"
    assert geo_df.columns[1] == "NT_1d"
...
def test_get_df_dose_list(file_path=file_path):
    # test geomean
    df_dose_list_geomean = get_df_dose_list(
        ["1h", "NT"],
        get_geo_df(
            file_path, filetype="geomean", doses=["1h", "NT"], timepoints=["1d"]
        ),
    ...
    # test geosem
    df_dose_list_geosem = get_df_dose_list(
        ["1h", "NT"],
        get_geo_df(file_path, filetype="geoSEM", doses=["1h", "NT"], timepoints=["1d"]),
    )
