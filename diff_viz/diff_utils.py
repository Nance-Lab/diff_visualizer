...
def get_experiment(date, donor, DIV, stimulus, level):
    ...
    """
    experiment = (
        date + "_" + donor + "_" + DIV + "_" + stimulus + "_" + level
    )  # Title of the Traj_csv folder without '_Traj_csv'
...
def get_csvs(data_path, filetype):
    ...
    geo_list = [
        j for j in listdir(data_path) if isfile(join(data_path, j)) and filetype in j
    ]
    ...
    if filetype == "geomean":
        ...
        for dose in doses:
            for time in timepoints:
                csv_list = [
                    file for file in geo_dict[dose][0] if dose in file and time in file
                ]
                temp_csv_df = []
            ...
            for time in timepoints:
                ...
                for csv in csv_list:
                    ...
                    if sum(dose and time in csv for csv in csv_list) > 1:
                        ...
                        csv_df = pd.concat(temp_csv_df, axis=1)
                        csv_df[dose + "_" + time] = csv_df.mean(axis=1)
                        geo_df_full = csv_df.drop(
                            columns=csv_df.columns[:ndrop], axis=1
                        )
